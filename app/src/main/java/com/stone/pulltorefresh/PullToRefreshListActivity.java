...
/**
 * 使用 com.handmark.pulltorefresh.library.PullToRefreshListView
 * 当显示到初始顶端时，下拉(比如此时在onRefreshListener中请求最新数据)，加载最新数据到顶部
 */
public final class PullToRefreshListActivity extends ListActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		mPullRefreshListView.setOnRefreshListener(new OnRefreshListener<ListView>() {
			...
			public void onRefresh(PullToRefreshBase<ListView> refreshView) {
				...
				// Update the LastUpdatedLabel 最后更新于 xx 时间点
		...
		registerForContextMenu(actualListView);//注册上下文菜单
	...
	@Override //menu 在3.0以上，会加到actionBar上 onCreateOptionsMenu 创建menu，只执行一次
	...
	@Override //上下文菜单  需要注册 registerForContextMenu(view) 到一个view上  长按触发
	...
	@Override //每次弹出menu前都会调用一次  用于动态改变menu菜单项
